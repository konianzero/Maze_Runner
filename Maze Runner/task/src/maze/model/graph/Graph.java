...
import static maze.model.graph.CellState.BORDER;
import static maze.model.graph.CellState.WALL;
...
public class Graph {
    ...
    private void addVertex(int row, int col) {
        if (isHorizontalBorder(col) || isVerticalBorder(row)) {
            createVertex(row, col, BORDER);
//            addCell(row, col);
        } else if (isOdd(col)) {
            if (isOdd(row)) {
                createVertex(row, col, WALL);
            }
        }
     ...
     */
    private void createVertex(int row, int col, CellState state) {
     ...
     * @param col cell x coordinate
     */
    private void createEdge(int from, int to, int row, int col) {
